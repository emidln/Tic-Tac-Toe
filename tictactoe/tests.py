...
class LogicTests(TestCase):
    ...
    def test_move_3_me_one_side(self):
        ...
        self.assertEqual(eg(self.b), True)
        self.assertEqual(bool([x for x in self.b.taken() if self.b[x] == COMPUTER if x != 4 if x in [0,2,6,8]]), True)
    ...
    def test_corner(self):
        def t(f):
            ...
            self.assertEqual(f(self.b), True)
            self.assertEqual(len([x for x in self.b.taken() if self.b[x] == COMPUTER if x in [0,2,6,8]]), 2) 
        t(corner)
        t(eg)
...
# further, we need to verify that the messages shown in the appropriate fields are in fact correct
# the only things tested here are response codes. Manual validation has shown that the code is correct, but that isn't always likely to be good enough.
